...
#include	<stdint.h>		// Standard type defs
	...
	struct DHCP_HEADER {
		...
		unsigned char Servername[ SERVERNAME_LENGHT ];
		unsigned char Boot_Filename[ BOOT_FILENAME_LENGHT ];
		unsigned long Magic_Cookie;
		char Options[ OPTION_LENGHT ];
	};

	#define DHCP_REQUEST_TIMEOUT		1000 // in 1/100 sekunden
	#define DHCPC_MAX_RETRY             10   // -1 -> unbegrenzte Anzahl Versuche

	#define DHCP_HEADER_LENGHT			sizeof( struct DHCP_HEADER )
