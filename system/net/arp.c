 ...
 */
void arp( int packet_lenght, char *ethernetbuffer)
	{
		unsigned char i,a;
		...
		switch ( ntohs( ARP_packet->ARP_Opcode ) ) {
			
			case 0x0001:		// check mal ob das für mich ist !
								...
								break;
			case 0x0002:		// check mal ob das für mich ist !
								for( i = 0 ; i < MAX_ARPTABLE_ENTRYS ; i++ )
								{
...
int GetIP2MAC( long IP, char * MACbuffer )
	{
		...
		char * ethernetbuffer;
		ethernetbuffer = (char*) __builtin_alloca (( size_t ) ETHERNET_HEADER_LENGTH + ARP_HEADER_LENGHT );
		...
		ARP_packet->HWtype = htons( 0x0001 );
		// Protocoltype = IP
		...
		for( i = 0 ; i < 6 ; i++){	
				...
				ETH_packet->ETH_sourceMac[i] = mymac[i]; }
		ETH_packet->ETH_typefield = htons( 0x0806 );
		...
		CLOCK_SetCountdownTimer( timer, 100 , MSECOUND );
