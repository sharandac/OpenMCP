/*! \file dns.c \brief Stellt Aufloesung von Hostnmen nach IP (dns.c) */
//***************************************************************************
//*            dns.c
//*
...
/**
 * \ingroup network
 * \defgroup DNS Stellt Aufloesung von Hostnmen nach IP (dns.c)
...
{		
		...
		// udp-puffer anlegen
		char * udpbuffer;
		udpbuffer = ( char*) __builtin_alloca (( size_t ) DNS_BUFFER_LENGHT );
		...
		DNS_question->Flags = htons( 0x0100 );
		DNS_question->Questions = htons( 1 );
