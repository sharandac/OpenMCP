...
volatile unsigned char TX_D1_state = TX_complete;

char TX_D1_Buffer[ TX_Bufferlen ];
char RX_D1_Buffer[ RX_Bufferlen ];
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_D1_init( void )
{
	// FIFO reservieren für RX und TX
	RX_D1_fifo = Get_FIFO( RX_D1_Buffer, RX_Bufferlen );
	TX_D1_fifo = Get_FIFO( TX_D1_Buffer, TX_Bufferlen );

	// TX_state auf complete setzen, da ja nix gesendet wurde
	TX_D1_state = TX_complete;
	...
	USART_Format_Set( &USARTD1 , USART_CHSIZE_8BIT_gc, USART_PMODE_DISABLED_gc, 0 );
	/* Set Baudrate to 9600 bps */
	USART_Baudrate_Set( &USARTD1 , ( UBRR_VAL ) , 0 );
...
ISR( USARTD1_DRE_vect )
{
	...
	if ( Get_Bytes_in_FIFO ( TX_D1_fifo ) != 0 )
	{
		USART_PutChar( &USARTD1 , Get_Byte_from_FIFO ( TX_D1_fifo ) );
	}
	else
	{
		// Wenn nicht war es das letzte Byte im Puffer, dann diesen Interrupt sperren und TX-Interrupt freigeben
		USART_TxdInterruptLevel_Set( &USARTD1 , USART_TXCINTLVL_HI_gc );
		USART_DreInterruptLevel_Set( &USARTD1 , USART_DREINTLVL_OFF_gc );
	}
}
/* -----------------------------------------------------------------------------------------------------------*/
...
ISR( USARTD1_RXC_vect )
{
	...
	Byte = USART_GetChar( &USARTD1 );
	
	if ( Get_Bytes_in_FIFO ( RX_D1_fifo ) < RX_Bufferlen )
		Put_Byte_in_FIFO ( RX_D1_fifo, Byte );
}
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_D1_SendByte( char Byte )
{
	char temp;
	...
	if ( Get_Bytes_in_FIFO ( TX_D1_fifo ) == 0 )
	{
		...
		if ( TX_D1_state == TX_sending )
		{			
			// Byte in Buffer schreiben
			Put_Byte_in_FIFO ( TX_D1_fifo, Byte );
			
			// Buffer Empty Interrupt freigeben, TX_complete sperren
			USART_TxdInterruptLevel_Set( &USARTD1 , USART_TXCINTLVL_OFF_gc );
			USART_DreInterruptLevel_Set( &USARTD1 , USART_DREINTLVL_HI_gc );
		}
		else
		{
			// Buffer Empty sperren, TX_complete freigeben
			USART_PutChar( &USARTD1, Byte );
			// Buffer Emty sperren, TX_complete freigeben
			USART_TxdInterruptLevel_Set( &USARTD1 , USART_TXCINTLVL_HI_gc );
			USART_DreInterruptLevel_Set( &USARTD1 , USART_DREINTLVL_OFF_gc );

			TX_D1_state = TX_sending;
		}
	}
	else
	{	
		...
		while ( 1 )
		{
			...
			if ( Get_Bytes_in_FIFO ( TX_D1_fifo ) < TX_Bufferlen ) break;
...
char UART_D1_GetByte( void )
{
	...
	if ( Get_Bytes_in_FIFO ( RX_D1_fifo ) != 0 )
		Byte = Get_Byte_from_FIFO ( RX_D1_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_D1_GetBytesinRxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	BytesInBuffer = Get_Bytes_in_FIFO ( RX_D1_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_D1_GetBytesinTxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	// Anzahl der Bytes holen
	BytesInBuffer = Get_Bytes_in_FIFO ( TX_D1_fifo );
