...
volatile char TX_C0_state = TX_complete;

char TX_C0_Buffer[ TX_Bufferlen ];
char RX_C0_Buffer[ RX_Bufferlen ];
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_C0_init( void )
{
	// FIFO reservieren für RX und TX
	RX_C0_fifo = Get_FIFO( RX_C0_Buffer, RX_Bufferlen );
	TX_C0_fifo = Get_FIFO( TX_C0_Buffer, TX_Bufferlen );

	// TX_state auf complete setzen, da ja nix gesendet wurde
	TX_C0_state = TX_complete;
...
ISR( USARTC0_DRE_vect )
{
	...
	if ( Get_Bytes_in_FIFO ( TX_C0_fifo ) != 0 )
	{
		USART_PutChar( &USARTC0 , Get_Byte_from_FIFO ( TX_C0_fifo ) );
	}
	else
	{
		// Wenn nicht war es das letzte Byte im Puffer, dann diesen Interrupt sperren und TX-Interrupt freigeben
		USART_TxdInterruptLevel_Set( &USARTC0 , USART_TXCINTLVL_HI_gc );
...
ISR( USARTC0_RXC_vect )
{
	...
	Byte = USART_GetChar( &USARTC0 );
	
	if ( Get_Bytes_in_FIFO ( RX_C0_fifo ) < RX_Bufferlen )
		Put_Byte_in_FIFO ( RX_C0_fifo, Byte );
}
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_C0_SendByte( char Byte )
{
	char temp;
	...
	if ( Get_Bytes_in_FIFO ( TX_C0_fifo ) == 0 )
	{
		...
		if ( TX_C0_state == TX_sending )
		{			
			// Byte in Buffer schreiben
			Put_Byte_in_FIFO ( TX_C0_fifo, Byte );
			
			// Buffer Empty Interrupt freigeben, TX_complete sperren
			USART_TxdInterruptLevel_Set( &USARTC0 , USART_TXCINTLVL_OFF_gc );
			USART_DreInterruptLevel_Set( &USARTC0 , USART_DREINTLVL_HI_gc );
		}
		else
		{
			// Buffer Empty sperren, TX_complete freigeben
			USART_PutChar( &USARTC0, Byte );
			// Buffer Emty sperren, TX_complete freigeben
			USART_TxdInterruptLevel_Set( &USARTC0 , USART_TXCINTLVL_HI_gc );
			USART_DreInterruptLevel_Set( &USARTC0 , USART_DREINTLVL_OFF_gc );

			TX_C0_state = TX_sending;
		}
	}
	else
	{	
		...
		while ( 1 )
		{
			...
			if ( Get_Bytes_in_FIFO ( TX_C0_fifo ) < TX_Bufferlen ) break;
...
char UART_C0_GetByte( void )
{
	...
	if ( Get_Bytes_in_FIFO ( RX_C0_fifo ) != 0 )
		Byte = Get_Byte_from_FIFO ( RX_C0_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_C0_GetBytesinRxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	BytesInBuffer = Get_Bytes_in_FIFO ( RX_C0_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_C0_GetBytesinTxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	// Anzahl der Bytes holen
	BytesInBuffer = Get_Bytes_in_FIFO ( TX_C0_fifo );
