...
volatile unsigned char TX_E1_state = TX_complete;

char TX_E1_Buffer[ TX_Bufferlen ];
char RX_E1_Buffer[ RX_Bufferlen ];
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_E1_init( void )
{
	// FIFO reservieren für RX und TX
	RX_E1_fifo = Get_FIFO( RX_E1_Buffer, RX_Bufferlen );
	TX_E1_fifo = Get_FIFO( TX_E1_Buffer, TX_Bufferlen );

	// TX_state auf complete setzen, da ja nix gesendet wurde
	TX_E1_state = TX_complete;
	...
	USART_Baudrate_Set( &USARTE1 , ( UBRR_VAL ) , 0 );
...
{
	USART_TxdInterruptLevel_Set( &USARTE1 , USART_TXCINTLVL_OFF_gc );
	TX_E1_state = TX_complete;	
}
...
ISR( USARTE1_DRE_vect )
{
	...
	if ( Get_Bytes_in_FIFO ( TX_E1_fifo ) != 0 )
	{
		USART_PutChar( &USARTE1 , Get_Byte_from_FIFO ( TX_E1_fifo ) );
	}
	else
	{
		// Wenn nicht war es das letzte Byte im Puffer, dann diesen Interrupt sperren und TX-Interrupt freigeben
		USART_TxdInterruptLevel_Set( &USARTE1 , USART_TXCINTLVL_HI_gc );
		USART_DreInterruptLevel_Set( &USARTE1 , USART_DREINTLVL_OFF_gc );
	}
}
/* -----------------------------------------------------------------------------------------------------------*/
...
ISR( USARTE1_RXC_vect )
{
	...
	Byte = USART_GetChar( &USARTE1 );
	
	if ( Get_Bytes_in_FIFO ( RX_E1_fifo ) < RX_Bufferlen )
		Put_Byte_in_FIFO ( RX_E1_fifo, Byte );
}
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_E1_SendByte( char Byte )
{
	char temp;
	...
	if ( Get_Bytes_in_FIFO ( TX_E1_fifo ) == 0 )
	{
		...
		if ( TX_E1_state == TX_sending )
		{			
			// Byte in Buffer schreiben
			Put_Byte_in_FIFO ( TX_E1_fifo, Byte );
			
			// Buffer Empty Interrupt freigeben, TX_complete sperren
			USART_TxdInterruptLevel_Set( &USARTE1 , USART_TXCINTLVL_OFF_gc );
			USART_DreInterruptLevel_Set( &USARTE1 , USART_DREINTLVL_HI_gc );
		}
		else
		{
			// Buffer Empty sperren, TX_complete freigeben
			USART_PutChar( &USARTE1, Byte );
			// Buffer Emty sperren, TX_complete freigeben
			USART_TxdInterruptLevel_Set( &USARTE1 , USART_TXCINTLVL_HI_gc );
			USART_DreInterruptLevel_Set( &USARTE1 , USART_DREINTLVL_OFF_gc );

			TX_E1_state = TX_sending;
		}
	}
	else
	{	
		...
		while ( 1 )
		{
			...
			if ( Get_Bytes_in_FIFO ( TX_E1_fifo ) < TX_Bufferlen ) break;
...
char UART_E1_GetByte( void )
{
	...
	if ( Get_Bytes_in_FIFO ( RX_E1_fifo ) != 0 )
		Byte = Get_Byte_from_FIFO ( RX_E1_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_E1_GetBytesinRxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	BytesInBuffer = Get_Bytes_in_FIFO ( RX_E1_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_E1_GetBytesinTxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	// Anzahl der Bytes holen
	BytesInBuffer = Get_Bytes_in_FIFO ( TX_E1_fifo );
