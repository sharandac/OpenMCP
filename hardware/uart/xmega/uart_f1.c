...
volatile unsigned char TX_F1_state = TX_complete;

char TX_F1_Buffer[ TX_Bufferlen ];
char RX_F1_Buffer[ RX_Bufferlen ];
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
void UART_F1_init( void )
{
	// FIFO reservieren für RX und TX
	RX_F1_fifo = Get_FIFO( RX_F1_Buffer, RX_Bufferlen );
	TX_F1_fifo = Get_FIFO( TX_F1_Buffer, TX_Bufferlen );

	// TX_state auf complete setzen, da ja nix gesendet wurde
	TX_F1_state = TX_complete;

	/* PIN3 (TXD0) as output. */
	PORTC.DIRSET = PIN7_bm;
	/* PC2 (RXD0) as input. */
	PORTC.DIRCLR = PIN6_bm;
	/* USARTF1, 8 Data bits, No Parity, 1 Stop bit. */
	USART_Format_Set( &USARTF1 , USART_CHSIZE_8BIT_gc, USART_PMODE_DISABLED_gc, 0 );
	/* Set Baudrate to 9600 bps */
	USART_Baudrate_Set( &USARTF1 , ( UBRR_VAL ) , 0 );
...
ISR( USARTF1_DRE_vect )
{
	...
	{
		USART_PutChar( &USARTF1 , Get_Byte_from_FIFO ( TX_F1_fifo ) );
	}
	else
	{
...
ISR( USARTF1_RXC_vect )
{
	...
	Byte = USART_GetChar( &USARTF1 );
	
	if ( Get_Bytes_in_FIFO ( RX_F1_fifo ) < RX_Bufferlen )
		Put_Byte_in_FIFO ( RX_F1_fifo, Byte );
}
...
char UART_F1_GetByte( void )
{
	...
	if ( Get_Bytes_in_FIFO ( RX_F1_fifo ) != 0 )
		Byte = Get_Byte_from_FIFO ( RX_F1_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_F1_GetBytesinRxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	BytesInBuffer = Get_Bytes_in_FIFO ( RX_F1_fifo );
 ...
 */
/* -----------------------------------------------------------------------------------------------------------*/
int UART_F1_GetBytesinTxBuffer( void )
{
	int BytesInBuffer=1;
	char temp;
	...
	// Anzahl der Bytes holen
	BytesInBuffer = Get_Bytes_in_FIFO ( TX_F1_fifo );
