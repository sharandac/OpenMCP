...
//****************************************************************************/
///	\addtogroup software
///	\defgroup CRON Der Crondienst (cron.c)
...
char cron_lasttime_mm;
char cron_lasttime_hh;
...
void CRON_init( void )
{
	...
	cron_lasttime_hh = nowtime.hh;
	cron_lasttime_mm = nowtime.mm;
...
void CRON_thread( void )
{
	...
	if ( cron_lasttime_hh != nowtime.hh || cron_lasttime_mm != nowtime.mm )
	{
		cron_lasttime_hh = nowtime.hh;
		cron_lasttime_mm = nowtime.mm;
...
int CRON_checkhit( int entry )
{
	...
	{
		if ( cron_lasttime_hh % ( crontable[ entry ].hh*-1 ) == 0 )
			HHhit = 1 ;
	}
	else if ( crontable[ entry ].hh == cron_lasttime_hh )
		HHhit = 1 ;
	
	if ( crontable[ entry ].mm < 0 )
	{
		if ( cron_lasttime_mm % ( crontable[ entry ].mm * -1 ) == 0 )
			MMhit = 1 ;
	}
	else if ( crontable[ entry ].mm == cron_lasttime_mm )
		MMhit = 1 ;
...
int CRON_pharseentry( char * cronstring, char ** argv )
{
	...
	argc = SHELL_pharse( cronstring, argv, MAX_CRON_ARGC );
	return( argc );
}
