 ...
 *
 * \ingroup httpd2
 * \addtogroup cgibin CGI-BIN Programme und Funktionen (cgi-bin.c)
...
/*------------------------------------------------------------------------------------------------------------*/
/*!\brief Registriert eine CGI-Funktion
 * \code
...
int check_cgibin( void * pStruct )
{
	...
	for( i = 0 ; i < MAX_CGI_ENTRYS ; i++ )
	{
		...
		{
			filename = http_request->GET_FILE + strlen( http_request->GET_FILE ) - 1;
			while( *filename != '.' )
				filename--;
			filename++;

			if ( !strcmp_P( filename , PSTR("cgi") ) )
				filename = filetype;
