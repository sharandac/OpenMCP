/* MenuInterpreter
  Version 1.5
  (c) 2009-2010, 2012 by Malte Marwedel
...
unsigned char menu_language;

void menu_language_set(unsigned char id) {
	if (id < MENU_LANGUAGES_MAX) {
		menu_language = id;
	}
#ifdef DEBUG
    else {
		printf("Error: language %i is out of range\n", id);
	}
#endif
}
#endif
...
#ifdef DEBUG
			printf("Multilang %i base address changed to: %i\n", menu_language, baseaddr);
#endif
		}
#endif
...
void menu_label(void) {
	...
	py += (SCREENPOS)(lpxy & 0x0F) << 8;
#endif
	unsigned char storage = (menu_byte_get_next()>>MENU_OPTIONS_STORAGE) & 3;
...
static void menu_basicbutton(unsigned char hasfocus, unsigned char offx, unsigned char offy) {
	...
	unsigned char storage = (options>>MENU_OPTIONS_STORAGE) & 3;
	MENUADDR textaddr  = menu_assemble_addr();
	unsigned char fonts = menu_byte_get_next();
#ifdef DEBUG
